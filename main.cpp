...
void printBoard(string board[8][8])
{
	...
	for (int i = 0; i < 64; i++)
	{
		printCell((i % 8 + 1) * 5, 1 + 3 * (i / 8), board[i / 8][i % 8][1], !bool((board[i / 8][i % 8][0] - 43) / 2), ((i / 8 % 2) * !bool(i % 2)) + (((i + 8) / 8 % 2) * bool(i % 2)));
	}
...
bool checkMove(int move[4], string board[8][8], bool side, int checkers[2], bool &beated)
{
	...
	// Make king
	if (((move[2] == 0 && side) || (move[2] == 7 && !side)) &&
	...
	if (board[move[0]][move[1]][1] == 'D')
	{
		...
		{
			if (board[i][j][0] == (!side * 45 + side * 43))
				return false; // If found own checker, stop (the move is prohibited)

			if (board[i][j][0] == (side * 45 + !side * 43) && ++rivalCounter > 1)
		...
		{
			if (board[cells[i][0]][cells[i][1]][0] == (side * 45 + !side * 43))
			{
				board[cells[i][0]][cells[i][1]] = "  "; // Remove rival checker
				checkers[!side]--;						// Decrease rival checkers count
	...
	else
	{
		...
		if (abs(move[3] - move[1]) == 2 && abs(move[2] - move[0]) == 2 &&
			board[(move[0] + move[2]) / 2][(move[1] + move[3]) / 2][0] == (side * 45 + !side * 43))
		{
		...
		if (!(abs(move[3] - move[1]) == 1 && abs(move[2] - move[0]) == 1))
			return false;
	}
...
void findNextBeat(int move[4], string *moves, int movesSize, string board[8][8], bool side, int checkers[2])
{
		...
		while (true)
		{
			...
			for (int i = -1; i <= 1; i += 2)
			{
				for (int j = -1; j <= 1; j += 2)
				{
					...
					{
						if (board[currPos[1] + i][currPos[0] + j][0] == (side * 45 + !side * 43) &&
			...
			if (availMovesSize > 0)
			{
				...
				while (running)
				{
					...
					{
						gotoXY(35, 29 + i);
						cout << char(availMoves[i][1] + 97) << 8 - availMoves[i][0];
					}
				...
				if (((currPos[1] == 0 && side) || (currPos[1] == 7 && !side)) &&
	...
	else
	{
		...
		while (true)
		{
			...
			// Up-Right
			if (!(prevPos[1] < currPos[1] && prevPos[0] > currPos[0]))
			{
				for (int i = currPos[0] + 1, j = currPos[1] - 1; i < 8 && j > -1; i++, j--)
				{
					if (board[j][i][0] == (!side * 45 + side * 43))
						break; // If find own checker, stop
					if (board[j][i][0] == (side * 45 + !side * 43))
					{ // Find the closest rival checker
						if (board[j - 1][i + 1] == "  ")
						{ // Check next cell after the cell with rival checker to move to
							rival[0] = j;
							rival[1] = i;
							for (++i, --j; i < 8 && j > -1; i++, j--)
							{ // Go further and gather available cells to move (beat)
								if (board[j][i] == "  ")
								{
...
bool gameProcess(string players[2], float score[2], int &draws)
{
	...
	while (true)
	{
		...
		else
		{
			...
			if (!checkMove(move, board, side, checkers, beated))
			{
				continue;
			}
		...
		if (checkers[0] * checkers[1] == 0)
		{
			...
			gotoXY(9, 10);
			cout << "------- Game over -------";
			gotoXY(21 - players[!side].length() / 2, 12);
			cout << players[!side];
			gotoXY(14, 13);
			cout << "is victorious!";
		...
		if (checkers[0] * checkers[1] == 0 || draw)
		{
			// Draw handler
			if (draw)
			{
...
void saveGameToFile(string players[2], float score[2], int draws, int games)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		stats["players"][players[i]]["all_wins"] = int(int(stats["players"][players[i]]["all_wins"]) + int(score[i] - 0.5 * int(draws)));
		stats["players"][players[i]]["all_loses"] = int(int(stats["players"][players[i]]["all_loses"]) + int(score[1 - i] - 0.5 * int(draws)));
		stats["players"][players[i]]["games_count"] = int(int(stats["players"][players[i]]["games_count"]) + games);
	}
...
void newGame()
{
	...
	cout << "------------------- New Game -----------------";
	gotoXY(9, 13);
	cout << "| White's player name: |                     |";
	gotoXY(9, 14);
	cout << "|----------------------|---------------------|";
	gotoXY(9, 15);
	cout << "| Black's player name: |                     |";
	gotoXY(9, 16);
	cout << "----------------------------------------------";
...
void showStats()
{
	...
	{
		gotoXY(15, 10);
		cout << "--- Not found any played game ---";
	}
	else
	{
		statsFileIn >> stats;
		...
		for (auto player : stats["players"].items())
		{
			...
			gotoXY(7, 16 + 2 * i);
			cout << "----------------------------------------------------";
			gotoXY(15 - player.key().length() / 2, 15 + 2 * i);
			cout << player.key();
			;
			gotoXY(28, 15 + 2 * i);
			cout << player.value()["games_count"];
			gotoXY(39, 15 + 2 * i);
			cout << player.value()["all_wins"];
			gotoXY(51, 15 + 2 * i++);
			cout << player.value()["all_loses"];
		}
	}
	_getch();
}
